<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        if ($Pkg->validate(true)) {
            die("Your name is " . $_POST["name"] . " and you are " . $_POST["age"] . " years old.");
        }
    }   
...
<html>
    <head>
        ...
        <title><?= $Environment->get("APP_NAME") ?? "App Name" ?> | CSRF Testing</title>
    ...
    <body>
        <div class="flex items-center justify-center min-h-screen bg-gray-100 p-6">
            <form action="" method="POST" class="w-full max-w-md bg-white rounded-lg shadow-md p-8">
                <?= $Pkg->load() ?>
                <h2 class="text-2xl font-bold mb-6 text-gray-800">CSRF Security Test</h2>
